...
const ViewportDownloadForm = ({
  ...
  const studyId = DicomMetadataStore.getStudyInstanceUIDs();
  const studyData = DicomMetadataStore.getStudy(studyId[0]);
  const firstInstance = studyData?.series[0]?.instances[0];
  const pName = utils.formatPN(firstInstance?.PatientName);
  const pAge = firstInstance?.PatientAge?.replace(/^0+/, '');
  const pSex = firstInstance?.PatientSex;
  const pMrn = firstInstance?.PatientID;
  const InstitutionalDepartmentName = firstInstance?.InstitutionalDepartmentName;
  const AcquisitionDate = firstInstance?.AcquisitionDate;
  const SeriesNumber = firstInstance?.SeriesNumber;
        ...
        >
          ...
          {activeViewportElement && (
            <div
              ...
              style={{
                height: viewportElementDimensions.height + viewportElementDimensions.height * 0.2,
              ...
              {showAnnotations && (
                <table style={{ width: '100%', marginBottom: 2 }}>
                  <tbody>
                    ...
                    <tr>
                      <td>Date: {AcquisitionDate}</td>
                      <td>Series No.: {SeriesNumber}</td>
                      <td>Type: {InstitutionalDepartmentName}</td>
                    </tr>
                  </tbody>
                </table>
              )}

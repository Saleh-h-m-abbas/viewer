...
The Arcova Viewer can be embedded in other web applications via it's [packaged
...
The quickest and easiest way to get the Arcova Viewer up and running is to embed
...
You tell the Arcova Viewer to use JSON by appending the `'?url='` query to the `/Viewer` route:

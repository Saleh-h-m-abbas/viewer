...
<ol start="2">
  <li>Create a JS Object or Function to hold the Arcova Viewer's configuration. Here are some
...
var componentRenderedOrUpdatedCallback = function() {
  console.log('Arcova Viewer rendered/updated');
};
...
The UMD build of the Arcova Viewer is a "light weight" build that only contains
...
> My application's styles are impacting the Arcova Viewer's look and feel. What
> can I do?
...
> We're trying to embed the Arcova Viewer into an existing React App, but seeing
> react-dom and react conflicts. What can we do?

If you are installing Arcova Viewer inside another react app, you may use `installViewer` as follows:
```jsx

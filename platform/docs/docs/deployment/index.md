...
The Arcova Viewer can be served as a stand-alone PWA ([progressive web
...
Deploying the Arcova Viewer as a stand-alone web application provides many
...
At the end of the day, a production Arcova Viewer instance is a collection of
...
build the Arcova Viewer in our
...
However, you can still embed the viewer using an iframe. You can utilize the iframe element to load the Arcova Viewer and establish communication with it using the postMessage API if needed.
Read more about how to use the iframe [here](./iframe.md).


## Data

The Arcova Viewer is able to connect to any data source that implements the [DICOM
...
If you have an existing archive and intend to host the Arcova Viewer at the same
...
#### What if I intend to host the Arcova Viewer at a different domain?
...
You tell the Arcova Viewer to use JSON by using the `dicomjson` datasource and
...
The Arcova Viewer can be configured to work with authorization servers that
